...
def generate_launch_description():
    ...
    franka_semantic_xacro_file = os.path.join(
        get_package_share_directory('franka_sonde'),
        'urdf', 'robot_complet.srdf.xacro'
    )

    robot_description_semantic_config = Command(
        [FindExecutable(name='xacro'), ' ',
         franka_semantic_xacro_file, ' robot_type:=fr3',
         ' arm_prefix:=', '', ' no_prefix:=true',
         ' hand:=', 'false',  # notre propre outil à la place
         ' ee_id:=', 'none'  # pas de gripper Franka
        ]
    )
    ...
    kinematics_yaml = load_yaml(
        'fr3_sonde_moveit_config', 'config/kinematics.yaml'
    )
    ...
    ompl_planning_yaml = load_yaml(
        'fr3_sonde_moveit_config', 'config/ompl_planning.yaml'
    )
    ompl_planning_pipeline_config['move_group'].update(ompl_planning_yaml)

    # Trajectory Execution Functionality
    moveit_simple_controllers_yaml = load_yaml(
        'fr3_sonde_moveit_config', 'config/fr3_sur_rail_controllers.yaml'
    )
    moveit_controllers = {
    ...
    ros2_controllers_path = os.path.join(
        get_package_share_directory('fr3_sonde_moveit_config'),
        'config',
    ...
    # Load controllers
    load_controllers = []
    for controller in ['fr3_arm_controller',  'rail_controller', 'joint_state_broadcaster']:
